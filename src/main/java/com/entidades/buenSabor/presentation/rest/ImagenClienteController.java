...
public class ImagenClienteController {
    @Autowired
    private ImagenClienteService imageService; // Inyección de dependencia del servicio ImageService
    ...
    public ResponseEntity<?> uploadImages(
        ...
        try {
            return ResponseEntity.ok(imageService.uploadImage(files)); // Llama al método del servicio para subir imágenes
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> getAllById(@RequestParam("uuid") Set<String> uuids){
        try{
            Set<ImagenCliente> images = imageService.findByIds(uuids);
            if (images.isEmpty()){
